...
// Configuration with specific Max limits
const METRICS_CONFIG = [
    { name: 'Sharp Ratio', value: 3.12, max: 5 },
    { name: 'Profit Factor', value: 0.5, max: 5 }, // Low value example
    { name: 'Recovery Factor', value: 8.40, max: 10 },
];

// Helper: Maps value % to a specific solid color on the Red-Yellow-Green scale
const getColorForPercentage = (percentage: number) => {
    // 0% - 35%: Red Zone
    if (percentage <= 35) return '#ef4444'; // Red-500
    
    // 35% - 75%: Warning/Mid Zone (Yellow/Orange)
    if (percentage <= 75) return '#eab308'; // Yellow-500
    
    // 75%+: Healthy Zone (Green)
    return '#10b981'; // Emerald-500
};

// Transform data: Normalize to 0-100 scale and assign color
...
export const AdvancedMetrics = () => {
    return (
        // Updated container style to match PortfolioDistribution
            ...
            <div className="relative z-10 h-[180px] w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        {/* Domain 0-100 ensures bars are consistently scaled against their Max */}
